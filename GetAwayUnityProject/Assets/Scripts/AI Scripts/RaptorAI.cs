...
public class RaptorAI : MonoBehaviour {
	...
	private bool preyDetected, predatorDetected, avoidObstacle, setRotationPoint, addRotation, preySearch,rotatedToPatrolPoint;
	...
	//T-rex movement stuff
	private CharacterController controller;
	private float movespeed;
	private Vector3 velocity;
	public float gravity,  walkSpeed, runSpeed, turnDamping, escapeDistance;
	...
	//Stuff for T-rex idling after patrol points
	...
	//The amount by which the t-rex's size can vary, as a decimal value
	...
	void Update () {
		if (health.isDead ()){
			...
			//else if(predatorDetected){
			//	animation.CrossFade("run");
			//	escapePredator(currentTarget);
			//	Patrol();
			//}
	...
	#region escapePredator(GameObject predator)
	void escapePredator(GameObject predator){
		movespeed = runSpeed;
		...
		currentTargetPosition = transform.position + oppositeDirection;
		//transform.forward = transform.position - predator.transform.position;
	}
	#endregion
	...
	void OnTriggerStay(Collider target){
		if(!target.gameObject.Equals(this.gameObject)){
			...
			if(target.GetComponent<FoodChain>()){
				...
				else if(target.GetComponent<FoodChain>().getFoodChainLevel() > foodChainLevel){ //Target is higher on the food chain
				...
				if(Vector3.Angle(transform.forward, target.transform.position-transform.position) < avoidance && Vector3.Angle(transform.forward, currentTargetPosition-transform.position) < 5 && dinoToDestinationDistance > dinoToObstacleDistance ){
					//if(Vector3.Angle(transform.forward, -hit.normal) < 20){
	...
	void OnTriggerExit(Collider target){
		...
		if(target.GetComponent<FoodChain>()){
			...
			else if(target.GetComponent<FoodChain>().getFoodChainLevel() > foodChainLevel){
				predatorDetected = false;
			}
		}
	}
